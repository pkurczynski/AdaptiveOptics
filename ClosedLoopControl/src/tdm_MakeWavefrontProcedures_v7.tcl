...
proc generateBesselWavefront { } \
{
   ...
   #DEBUG
   #puts stdout "generateBesselWavefront: "
   #a.min theYCoords_um = theMin
   #a.max theYCoords_um = theMax
   #puts stdout \
   #"     theYCoords_um: min= [a.dump theMin] max= [a.dump theMax]"
   ...
   # COMPUTE THE GRADIENT OF THE WAVEFRONT IN POLAR COORDINATES
   # Wavefront:  xi(x,y) =  N J_v( x_vn * r/a ) * e^i*v*phi
   #                        N = 2/(a*sqrt(pi)*|J_v+1(x_vn)|)
   #
   # (for j:[0,9] the Bessel function is a J_0 and there is no angular
   # dependence.) For this special case, the gradient computed in polar
   # coordinates is given by
   #
   #            d xi / d r     = - N * (x_vn / a) * J_1( x_vn * r/a )
   #            d xi / d theta = 0
   #

   lookupBesselIndicesAndZero gEigenModeJIndex_int theV theN theXvn

   set theRadialScaleFactor [expr $theXvn / $gMembraneRadiusFactor_unit ]

   set theNDenomFact1 [expr $gMembraneRadiusFactor_unit * sqrt(3.14159)]

   # Bessel function factor here:
   set theNDenomFact2 1.0
   set theNDenom [expr theNDenomFact1 * theNDenomFact2]
   set theN [expr 2/theNDenom]

   

   # TRANSFORM THE GRADIENT FROM POLAR TO RECTANGULAR COORDINATES
   # Gradient of Wavefront is computed analytically in polar coordinates
   # and transformed to rectangular coordinates according to the following:
   #
   #  grad(xi)_x  = dr/dx * grad(xi)_r  +  dr/dx * grad(xi)_theta
   #  grad(xi)_y  = dr/dy * grad(xi)_r  +  dr/dy * grad(xi)_theta
   #
   # The above equations follow from the transformation law for a covariant
   # tensor of rank one (i.e. grad(xi) is a covariant vector ).
   #
   # Implementing the above equations leads to
   #
   #   grad(xi)_x = x/r * grad(xi)_r
   #   grad(xi)_y = y/r * grad(xi)_r
   #
   # There is no theta component for the case of v=0 eigenfunctions.
