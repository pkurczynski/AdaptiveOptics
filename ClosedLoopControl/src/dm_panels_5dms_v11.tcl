...
proc makerecon {} \
	...
	if { [a.rank Grds] == 3}\
		...
		a.mul ggg gmm = ggg
                ...
                # save poke matrix array to disk.
                # plk 9/23/2005
                #
                a.rows gmm = theNumRowsA
                a.cols gmm = theNumColsA

                set theFile "pokeMatrixAfterThresh.txt"
                puts stdout "makerecon:  saving array to file - $theFile"
                puts stdout \
                   "   [a.dump theNumRowsA] rows x [a.dump theNumColsA] cols"
                a.saveasc $gmm $theFile

                # old save poke matrix command.  Retain for compatibility
		a.saveasc $gmm gmm
	...
	a.transpose U = Ut
        ...
        # save SVD weight array to disk.
        # plk 9/23/2005
        #
        a.rows W = theNumRowsA
        a.cols W = theNumColsA

        set theFile "svdWeightBeforeThresh.txt"
        puts stdout "makerecon:  saving array to file - $theFile"
        puts stdout "   [a.dump theNumRowsA] rows x [a.dump theNumColsA] cols"
        a.saveasc $W $theFile
