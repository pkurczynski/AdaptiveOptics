...
# changed from 64...(25 Modes in reconstructor) plk 08/29/2005

#  gNumberOfActuatorsPerBin   Binning   # Modes in reconstructor
#  ------------------------   -------   ------------------------
#           64                  8x8            25
#           25                  5x5            64
#           16                  4x4           100
#            9                  3x3          ~125
set gNumberOfActuatorsPerBin 4
...
proc pokeBinnedActuators_quiet { } {
    ...
    # this line replaced with following line, to allow for operation
    # from non-zero Varray bias voltage.  Set all electrodes of the
    # array to the voltage given by the global variable Varray.  This
    # voltage is set in the ArrayControlGUI.  plk 8/23/2005
    #FlatDM
    setarray
   ...
   for { set i 0 } { $i < $XACT_LINE_LENGTH } { incr i $theIBinWidth } {
        for { set j 0 } { $j < $YACT_LINE_LENGTH } { incr j $theJBinWidth } {
             ...
             # this line disabled, replaced with the following line
             # to allow operation wrt to array bias voltage value.
             # plk 8/23/2005
             #setzero
             setarray
...
#------------------------------------------------------------------------
# setActuatorWeights_NoWeighting
#
...
#
# This mode weight scheme assigns a uniform, unit weight to all actuators.
#
...
# plk 08/30/2005
#------------------------------------------------------------------------
proc setActuatorWeights_NoWeighting {} \
{
    global gNumberOfActuatorsPerBin
    global gActuatorWeight
    global gModesWeightedPreviously
