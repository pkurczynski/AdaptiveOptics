...
proc generateBesselWavefront { } \
{
   ...
   set theMembraneRadius_pix $thePupilRadius_pix
   ...
   # create array for storing the wavefront OPD.
   ...
   a.cols theYCoord_pix = theNumYValuesA
   set theNumYValues [expr a.dump theNumYValuesA]

   # compute the wavefront value at each sampled point.  This cannot
   # be done with wavescope array functions; must compute each value
   # serially.
   for { set i 0 } { $i < $theNumXValues } { incr i } {
        for { set j 0 } { $j < $theNumYValues } { incr j } {
             ...
             # coordinate of the current sample point.
             ...
             # into the existing expression (use the real part only)
             ...
             a.repele theWav_pix theWavefrontOPD_pix i j = theWavefrontOPD_pix
        }

   }

   # scale the OPD to micron units.

   a.mul theWavefrontOPD_pix \
         $wlCalibrate(micronsPerPix) = gUserDefWavefrontOPD_float_um
