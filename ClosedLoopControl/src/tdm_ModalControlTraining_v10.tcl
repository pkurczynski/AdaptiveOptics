...
proc computeWavefrontByZernikeMethod {inGradV4 outWavefrontF outLaplacianF } \
{
  ...
  #DEBUG
  puts stdout "computeWavefrontAndLaplacianByZernikeMethod:"
  ...
  #DEBUG
  puts stdout "[a.info theGrad]"
  puts stdout "[a.info theZernikeCoeffsV2]"

  decomposeWavefrontIntoZernikeCoeffs theGrad \
                                      theZernikeCoeffsV2
...
#------------------------------------------------------------------------
# zern_make_lap_coefs     UNDER CONSTRUCTION.
#
#------------------------------------------------------------------------
proc zern_make_lap_coefs { inLapCoeffsV2 \
                           inMaxCol_Grid \
                           inMaxRow_Grid \
                           inPupilXCenter_Grid \
                           inPupilYCenter_Grid \
                           ...
                           outLaplacianF } \
{
    upvar $inLapCoeffsV2 theLapCoeffsV2
    upvar $inMaxCol_Grid theMaxCol_GridA
    upvar $inMaxRow_Grid theMaxRow_GridA
    upvar $inPupilXCenter_Grid thePupilXCenter_GridA
    upvar $inPupilYCenter_Grid thePupilYCenter_GridA
    upvar $inPupilRadius_Grid thePupilRadius_GridA
    upvar $outLaplacianF theLaplacianF
    ...
    for { set i 0 } { $i < $theMaxCol_Grid } { incr i } {
        for { set j 0 } { $j < $theMaxRow_Grid } { incr j} {

             set XR [expr $i - $thePupilXCenter_Grid]
             set YR [expr $j - $thePupilYCenter_Grid]
             set R2 [expr $XR*$XR + $YR*YR]
             set R [expr sqrt($R2)]
             set th [expr atan2($YR,$XR)]

             set theValue 0
             evaluateZernikeSeriesAtPoint $R $th theLapCoeffsV2 theValue
