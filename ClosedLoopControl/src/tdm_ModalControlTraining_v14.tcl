...
#
#
# parameters:  inI, inJ   column, row values of binned actuator number.
#                         these indices range over:
#                         0,1,2...< Num. Binned Actuators Per Side -  1 >
#
#
# UNDER CONSTRUCTION
#
...
proc trainOneActuator {inI inJ } \
{
   ...
   set theElectrodeI [expr $theBAI * $gNAPerSide]
   set theElectrodeJ [expr $theBAJ * $gNAPerSide]
   a.make "<$theElectrodeI $theElectrodeJ>" = gCurrentTrainingPointIJV2
                                             ...
                                             outCalibPointInputDataF } \
{
    ...
    set theSettleTime_ms 7000
    #after $theSettleTime_ms

    #DEBUG
    dialog "Press OK to take one Measurement."
                                         ...
                                         inMembraneVoltage } \
{
    ...
    if {$inMembraneVoltage == 0 } \
    {
       ...
       # -----------------------------------------------------
       # activate these lines of code to send voltage to
       # the entire array, rather than just a single actuator
       # -----------------------------------------------------
       #set Varray $inActuatorVoltage
       #set Vm $inMembraneVoltage
    ...
    } else \
    {
       ...
       # ---------------------------------------------------------------
       # these commands send voltage to the ENTIRE array of the
       # device, not just a single actuator.  To get upward deformation,
       # sending voltage to the entire array is necessary.
       # ---------------------------------------------------------------
       ...
       puts stdout "sendCurrentTrainingVoltagesToDACs: Entire array energized!"
...
# completed.  Needs debugging.
#
# NOTE:  DEBUG:  compute Zernike decomposition in x,y (imag pix.)
# coordinates NOT in grid (i,j) coordinates.
#
# 7/24/2005
#------------------------------------------------------------------------
proc computeWavefrontByZernikeMethod {inGradV4 outWavefrontF outLaplacianF } \
{
  ...
  #=========================================================
  # THIS CODE DISABLED FOR DEBUG 7/24/2005
  ...
  # DEBUG 7/24/2005
  #
  # Compute Zernike decomposition in x,y coordinates
  # (imag. pix) NOT grid coordinates!
  #=========================================================
  a.v2toxy theCCPixelV2 = theMaxCol_ImagPixA theMaxRow_ImagPixA
  zern.make.surf.coefs theZernikeCoeffsV2 \
                       theMaxCol_ImagPixA \
                       theMaxRow_ImagPixA \
                       thePupilXCenterA \
  ...
  # this old procedure is obsolete.  It produces incorrect output.
  #computeLapCoeffsFromWavCoeffs theZernikeCoeffsV2 theLapCoeffsV2


   # ----------------------------------
   # Zernike Coefficient scaling factor
   # ...  May/may not be necessary!
   # ----------------------------------

   set theOneByPupilRadius_Grid 1.0

   # scaling factor = reciprocal of pupil radius? Maybe/not...
   #set theOneByPupilRadius_Grid [expr 1.0/$thePupilRadius_Grid]
   ...
   a.make "<$theOneByPupilRadius_Grid 0>" = thePupilRadiusIJV2
   a.v2toxy thePupilRadiusIJV2 = thePupilRadiusIA thePupilRadiusJA
   a.v2toxy theLapCoeffsV2 = theIndex theValue
   a.mul theValue -1 = theValue
   a.mul theValue thePupilRadiusIA = theValue
   a.xytov2 theIndex theValue = theLapCoeffsV2
  ...
  # DEBUG
  puts stdout "computeWavefrontByZernikeMethod:"
  puts stdout "   Wavefront Zernike Coefficients:"
  puts stdout "          [a.dump theZernikeCoeffsV2]"
  puts stdout "   Laplacian Zernike Coefficients:"
  puts stdout "          [a.dump theLapCoeffsV2]"
...
#
# THIS PROCEDURE PRODUCES INCORRECT OUTPUT AND IS OBSOLETE.  IT HAS BEEN
# SUPERCEDED BY: computeLaplacianOfZernikeSeries   SEE SCRIPTS IN THE FILE
# tdm_ModalControlZernike.tcl
#
...
#                        <index value>.  Index from 1...N
#
# called by:  computeWavefrontByZernikeSeries  (not anymore.  see NOTE above)
#
# completed. See ...wavescope\kraken\Zernikes\zernikeprocs_v1.tcl
#
# 7/8/2005
#------------------------------------------------------------------------------
                                           ...
                                           outZernikeCoeffsV2 } \
{
   ...
   # gNumZernikes declared in tdm_ModalControlZernike.tcl.
   # value set in procedure initLapZernMatrix called by initDMTraining
   ...
   # Zernike coefficients so far do not include piston term, to which
   # the wfs is blind.  Add a piston term to the array with value zero,
   # so that the Zernike series has full range of coefficients for
   # z=1...z=gNumZernikes.
   a.make "<1 0>" = theZ1Coef
   a.copy theWavefrontZernikeCoeffsV2 = theNoPistonCoeffsV2
   a.make "<0 0>" $theNumberOfZernikeTerms = theWavefrontZernikeCoeffsV2
   a.repele theZ1Coef \
            theWavefrontZernikeCoeffsV2 \
            0 \
            = theWavefrontZernikeCoeffsV2

   a.rep theNoPistonCoeffsV2 \
         theWavefrontZernikeCoeffsV2 \
         1 \
         = theWavefrontZernikeCoeffsV2
