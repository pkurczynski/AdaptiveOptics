...
proc iterateOneWavefrontCorrection { i } \
{
	...
	update
        ...
        puts stdout "\tIteration number: $i"
...
proc computeDriveVoltagesAndSendToDACs { i } \
{
	...
	if { $i == 0 } {
        ...
        # add the current bias array voltage to the
        # drive signal.  This allows operation from
        # a non-zero array bias voltage. To operate
        # from flat position, with TE unpowered,
        # ensure that Varray = 0 !!
        # plk 08/23/2005
        #
        # THIS MAY BE A MISTAKE.  Drive VOLTAGES ARE
        # COMPUTED IN ftov BY TAKING INTO ACCOUNT BIAS
        # VOLTAGE.  DO NOT CORRECT FOR BIAS VOLTAGE TWICE.
        # plk 08/26/2005
        # a.add Drive $Varray = Drive


        # DEBUG:  Drive voltage statistics
        a.min Drive = theMin
        a.max Drive = theMax
        puts stdout "computeDriveVoltagesAndSendToDACs:"
        puts stdout \
