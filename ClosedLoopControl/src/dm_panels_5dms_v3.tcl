...
# Modified tor transpatene electrode device; binned electrodes 12/16/2004
# plk
...
# proc calcLap
# 
# Calculate laplacian (wrapper for lap function)
#
### For DMs: BMC 140, Membrane 256, BMC 1024, Membrane 1024
#
#---------------------------------------------------------------------------
...
proc ftov { fracar voltar } {
    ...
    #DEBUG
    puts stdout "Now in ftov.  Device = $device"
    puts stdout "    fracar = $fracar; voltar =$voltar"

    if { $device == "tdm1024" } {
	upvar $voltar vt
	global MAX_VOLT deltaV Varray

	a.div $fracar 109 = ftemp
	a.abs $ftemp = abstemp
	a.div $ftemp $abstemp = sign
	a.sqrt $abstemp = rttemp
	#adjust above line to suit measured deflection vs voltage curve.
	a.mul $rttemp $sign = ftemp
	a.mul $ftemp $deltaV = temp
	a.add $Varray $temp = Vtemp
	a.limlow Vtemp 0 = Vtemp
	a.to Vtemp d = vt

        # code from here down to end of if {} was found commented
        # out plk 12/14/2004
...
# 	a.to    ftemp uc =  vt
	#	tftov $fracar $voltar
	#If transparent membrane device, call alternative ftov function
    } else {
	upvar $voltar vt
...
proc PokeAct { value } {
    ...
    # DEBUG
    # should ftov be called with volts or $volts ?
    # "originally" it was called with volts.
    # plk 12/14/2004
