...
#Desired changes:
#
# 1) Amend renaming routines to determine what renaming is necessary
#    Done; hdyson 27th Oct 03 
# 2) Add options to reconstructor to only use a subset of actuators
# 2b) Use subsets of actuators to evaluate reconstructor options
#    Done; hdyson 27th Oct 03 (still needs testing)
# 3) Closed loop: using any loop checks boxes for all.  Why?
#    Done; hdyson 27th Oct 03 (still needs testing)
# 4) Amend reconstructor to output number of valid modes found?
#    Potential solution implemented; hdyson 27th Oct 03 (still needs testing)
# 5) Why does 'display mode' window open off-screen?
#    Potential solution implemented; hdyson 27th Oct 03 (still needs testing)
# 6) Re-order actuators in poke DM screens to fit camera orientation
#    (ie reverse order)
###
...
#  #button .cl.f.clb.poke -text "Poke DM..." -command { DM_GUI }
  ...
  if {$device == "membrane256"} {
      ...
      button .cl.f.clb.loadar -text "Load Voltage file (see 'before using wavescope' file on desktop)" -command { load_array_gui }  
...
#Copies (float) ftemp to unsigned char vt
  ...
  puts stdout "max ftemp= [a.max $ftemp]"
  puts stdout "max vt= [a.max $vt]"
...
proc quiet_restricted { } {
  ...
  global Grad gvd CurDrv Drvs Grds MAX_ACT maskArray pokeFraction
  global wlCalibrate ACT_LINE_LENGTH
...
#  a.cut W $minw = Wmask
#  a.mul W Wmask = W
#Hack warning: above may not be a good idea...
#hdyson 14 Nov 03
